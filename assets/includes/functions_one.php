...
function Wo_UserData($user_id, $password = true)
{
    ...
    /*added by qasim*/
    $query_anonymous = "SELECT * FROM " . T_ANONYMOUS_USERS . " WHERE `user_id` = '{$user_id}'";
    $sql = mysqli_query($sqlConnect, $query_anonymous);
    if (mysqli_num_rows($sql)) {
        $fetched_data['anonymous'] = mysqli_fetch_assoc($sql);
    }
    /*added upto*/
...
function Wo_RegisterUser($registration_data, $invited = false)
{
    ...
    unset($registration_data['password']);
    $fields = '`' . implode('`,`', array_keys($registration_data)) . '`';
    $data = '\'' . implode('\', \'', $registration_data) . '\'';
    $query_3 = mysqli_query($sqlConnect, "INSERT INTO " . T_ANONYMOUS_USERS . " (`user_id`,{$fields}) VALUES ($user_id,{$data})"); //added by qasim
...
//added by qasim - Wo_UpdateAnonymousUserData
function Wo_UpdateAnonymousUserData($user_id, $update_data, $unverify = false)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $impload = implode(', ', $update);
    $query_one = " UPDATE " . T_ANONYMOUS_USERS . " SET {$impload} WHERE `user_id` = {$user_id} ";

    $query1 = mysqli_query($sqlConnect, $query_one);
    if ($unverify == true) {
        $query_two = " UPDATE " . T_ANONYMOUS_USERS . " SET `verified` = '0' WHERE `user_id` = {$user_id} ";
        @mysqli_query($sqlConnect, $query_two);
    }
    if ($query1) {
        cache($user_id, 'users', 'delete');
