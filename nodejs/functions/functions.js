...
function deleteCache(userid) {
    ...
    if (userid <= 100) {
        ...
        subfolder = '10000';
    } else if (userid <= 15000) {
        subfolder = '15000';
    } else if (userid <= 20000) {
        subfolder = '20000';
    } else if (userid <= 40000) {
        subfolder = '40000';
    } else if (userid <= 60000) {
        subfolder = '60000';
    } else if (userid <= 80000) {
        subfolder = '80000';
    } else if (userid <= 100000) {
        subfolder = '100000';
    } else if (userid <= 150000) {
        subfolder = '150000';
    } else if (userid <= 250000) {
        subfolder = '250000';
    } else if (userid <= 450000) {
        subfolder = '450000';
    } else {
        subfolder = 'all';
    }

    var filePath = "../../cache/users/" + subfolder + "/" + id + ".tmp";
...
class FunctionsUtils {
    async sanitizeJS(string) {
        ...
        // let linkReg = /(((https?:\/\/)|(www\.))[^\s]+)/g;
        // let urls = string.match(linkReg).map((url) => {
        //     links[url.replace(reg, (match)=>(map[match]))] = url;
        // });

        // string = string.replace(reg, (match)=>(map[match]));

        // Object.keys(links).map((value) => {
        //     string = string.replace(value, (match)=>(links[value]))
        // })
    ...
    async Wo_CensoredWords(ctx, text) {
        if (text && typeof(text) != 'undefined' && text != '' && ctx.globalconfig.censored_words.length > 0) {
            let ar = ctx.globalconfig.censored_words.split(",");
            ar.map(word => {
    ...
    async Wo_Markup(ctx, text) {
        ...
        text = await this.Wo_CensoredWords(ctx,text);
        return text;
    }
    ...
    async Wo_DisplaySharedFile(ctx, message_id, placement, isSticker) {
        ...
        if (isSticker && message['stickers'] != '' && message['stickers'] != null) {
            if (message['stickers'] != '') {
    ...
    async Wo_GetMedia(ctx, media) {
        ...
        } else if (ctx.globalconfig['backblaze_storage'] == 1) {
            return 'https://'+ctx.globalconfig['backblaze_bucket_name']+'.s3.'+ctx.globalconfig['backblaze_bucket_region']+'.backblazeb2.com/' + $media;
        }
