...
if ($_POST['type'] == 'add') {
	try {
		...
		$db->insert(T_USER_ADDRESS,array('name' => Wo_Secure($_POST['name']),
...
elseif ($_POST['type'] == 'delete') {
	try {
		...
		$db->where('id',$wo['address']->id)->delete(T_USER_ADDRESS);
		$response_data = array(
	        'api_status' => 200,
...
elseif ($_POST['type'] == 'edit') {
	try {
		...
		$db->where('id',$wo['address']->id)->update(T_USER_ADDRESS,array('name' => Wo_Secure($_POST['name']),
									                                'phone' => Wo_Secure($_POST['phone']),
									                                'city' => Wo_Secure($_POST['city']),
									                                'zip' => Wo_Secure($_POST['zip']),
									                                'address' => Wo_Secure($_POST['address']),
...
elseif ($_POST['type'] == 'get') {
	...
	$wo['addresses'] = $db->where('user_id',$wo['user']['user_id'])->orderBy('id','DESC')->get(T_USER_ADDRESS,$limit);
